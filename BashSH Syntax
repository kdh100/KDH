...
10. Debugging
	* 스크립트 구문 검사
		- bash -nv : 실행시키지 않고(n) 자세히 구문 검사(v)
		- bash -x : 실생시키되 스크립트의 각 행을 echo
11. 옵션 핸들링
	* 스크립트 내에서 옵션이 필요한 CMD 가 사용되어야 할 때 사용
		- 옵션 뒤에 :(콜론)을 붙이면 이 옵션은 인자값이 필요함을 의미
		- OPTARG : 유효한 옵션의 인자값
		- OPTIND : 기본값 1(옵션이 없는 상태), 옵션과 인자의 개수에 따라 1씩 증가
			# shift 와 함께 사용해서 기본 CMD 의 인자 필드로 변경할 수 있음
		- 예제
		-----------------------------------
		#!/bin/bash
		#   ssh -P 22 192.168.10.20

		Usage() {
		    echo "Usage: $0 [-p PORT] {IP}"
		    exit 1
		}
